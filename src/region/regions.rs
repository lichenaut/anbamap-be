...
impl RegionKeyphrases {
    pub fn get_region_vec(self) -> Vec<String> {
        ...
        region_vec.iter_mut().for_each(|s| if s.len() < 4 { quotationed_short_strings.push(format!("'{}'", s)); });
        region_vec.iter_mut().for_each(|s| if s.len() < 4 { quotationed_short_strings.push(format!("\"{}\"", s)); });
        ...
        let mut inc_strings = Vec::new();
        region_vec.iter().for_each(|s| if s.contains(" inc") { inc_strings.push(format!("{}, inc", &s[..s.len() - 3])); });
        region_vec.extend(inc_strings);
