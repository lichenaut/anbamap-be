...
pub async fn scrape_ei(
    ...
    media.extend(scrape_ei_releases(pool, "https://electronicintifada.net/blog").await?);

    Ok(())
}

pub async fn scrape_ei_releases(
    pool: &SqlitePool,
    url: &str,
    ...
    response = match look_between(
        ...
        "<p><a href=\"https://ei.org/news-releases/page/2/\" >".to_string(),
    )? {
